...
class cfg(cfg_common, cfg_dataset_default, cfg_model_gsanet):

	def __init__(self):
		cfg_common.__init__(self)
		cfg_dataset_default.__init__(self)
		cfg_model_gsanet.__init__(self)
		...
		self.data.root = '/home/mdisk2/hulei/MultiView/ADer/data/Real-IAD/realiad_1024'
		self.data.depth_root = '/home/mdisk2/hulei/MultiView/ADer/data/depth_masked'
		self.data.cam_root = '/home/mdisk2/hulei/MultiView/ADer/data/camera'
		self.data.use_sample = True
		self.data.views = []  # ['C1', 'C2', 'C3', 'C4', 'C5']
		self.data.cls_names = []
		
		# 选取训练数据类别
		self.data.cls_select = True
		...
		self.model_attn_args = dict(so2=1, f_dims=dict(se3=16), max_freq_h=1, max_freq_w=1)
		
		self.model = Namespace()
		self.model.name = 'gsanet'
		self.model.kwargs = dict(pretrained=False,
        						 ...
        						 attn_args = self.model_attn_args)
		...
		# ==> trainer
		self.trainer.name = 'GSANetTrainer'
