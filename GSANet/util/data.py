...
def rotate(image, angle, background_color):
...
def pil_loader_rot(path, angle):
    ...
    while not is_ok:
        try:
            ...
            rotated_img = rotate(img, angle, background_color)
            img = center_crop(rotated_img, img.size)
            is_ok = True
        except:
            time.sleep(1)
            continue
...
def pil_loader_L_rot(path, angle):
    wait_for_path(path)
...
def get_img_loader(loader_type):
    ...
    elif loader_type == 'pil_rot':
        return pil_loader_rot
    elif loader_type == 'pil_L_rot':
        return pil_loader_L_rot
    else:
