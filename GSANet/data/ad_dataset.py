...
class RealIAD(data.Dataset):
	def __init__(self, cfg, train=True, transform=None, target_transform=None):
		self.root = cfg.data.root
		self.cam_root = cfg.data.cam_root
		self.train = train
		self.transform = transform
		self.target_transform = target_transform
		self.depth_root = cfg.data.depth_root

		self.cls_names = cfg.data.cls_names
		self.cls_select = cfg.data.cls_select
		if not isinstance(self.cls_names, list):
			self.cls_names = [self.cls_names]
		if len(self.cls_names) == 0:
			cls_names = os.listdir(self.root)
		...
		self.loader = get_img_loader(cfg.data.loader_type)
		self.loader_target = get_img_loader(cfg.data.loader_type_target)
		self.loader_rot = get_img_loader(cfg.data.loader_type_rot)
		self.loader_target_rot = get_img_loader(cfg.data.loader_type_target_rot)
	...
	def __getitem__(self, index):
		if self.use_sample == True:
			...
			img_path, mask_path, cls_name, specie_name, anomaly, sam_mask_path = [], [], [], [], [], []
			img, img_mask, sam_mask, depth = [], [], [], []
			...
			for idx in selected_indices:
				img_path.append(sample[idx]['img_path'])
				sam_mask_path.append(sample[idx]['img_path'])
				mask_path.append(sample[idx]['mask_path'])
				cls_name.append(sample[idx]['cls_name'])
				specie_name.append(sample[idx]['specie_name'])
				anomaly.append(sample[idx]['anomaly'])
				img_path[-1] = f'{self.root}/{img_path[-1]}'
				img.append(self.loader(img_path[-1]))
    
				if anomaly[-1] == 0:
					img_mask.append(Image.fromarray(np.zeros((img[-1].size[0], img[-1].size[1])), mode='L'))
				else:
					img_mask.append(np.array(self.loader_target(f'{self.root}/{mask_path[-1]}')) > 0)
					img_mask[-1] = Image.fromarray(img_mask[-1].astype(np.uint8) * 255, mode='L')
     
				img[-1] = self.transform(img[-1]) if self.transform is not None else img[-1]
				img[-1] = img[-1].unsqueeze(0)
				img_mask[-1] = self.target_transform(
					img_mask[-1]) if self.target_transform is not None and img_mask[-1] is not None else img_mask[-1]
				img_mask[-1] = [] if img_mask[-1] is None else img_mask[-1].unsqueeze(0)

				depth_path = os.path.join(self.depth_root, sample[idx]['img_path'].replace('.jpg', '.png'))
				depth_i = cv2.imread(depth_path, cv2.IMREAD_UNCHANGED)
				depth_a, alpha = depth_i[:,:,:3], depth_i[:,:,3]
				depth_a[alpha == 0] = 0
    
				depth_tensor = torch.from_numpy(depth_a[:,:,0]).unsqueeze(0).unsqueeze(0).float()
				depth.append(depth_tensor)

			sample_anomaly = 0 if 1 not in anomaly else 1
			img = torch.cat(img, dim=0)
			img_mask = torch.cat(img_mask, dim=0)
			depth = torch.cat(depth, dim=0)
			cam_path = os.path.join(os.path.dirname(sample[idx]['img_path']), 'cam_extr.npy')
			cam_path = f'{self.cam_root}/{cam_path}'
			cam = torch.from_numpy(np.load(cam_path)).squeeze(0)
			selected_cam = cam[selected_indices]
   
			return {'img': img, 'img_mask': img_mask, 'cls_name': cls_name, 'anomaly': anomaly, 'img_path': img_path, 'sample_anomaly': sample_anomaly, 'cam': selected_cam, 'depth': depth}
		else:
			data = self.data_all[index]
			img_path, mask_path, cls_name, specie_name, anomaly = data['img_path'], data['mask_path'], data['cls_name'], data['specie_name'], data['anomaly']
			img_path = f'{self.root}/{img_path}'
