...
class GSANetTrainer(BaseTrainer):
	def __init__(self, cfg):
		super(GSANetTrainer, self).__init__(cfg)
	...
	def set_input(self, inputs):
		...
		self.imgs = inputs['img'].view(-1,inputs['img'].shape[2],inputs['img'].shape[3],inputs['img'].shape[4]).cuda()
		# sam生成的mask
		# self.sam_mask = inputs['sam_mask'].view(-1,inputs['sam_mask'].shape[2],inputs['sam_mask'].shape[3],inputs['sam_mask'].shape[4]).cuda()
    
		self.imgs_mask = inputs['img_mask'].view(-1,inputs['img_mask'].shape[2],inputs['img_mask'].shape[3],inputs['img_mask'].shape[4]).cuda()
		cam = inputs['cam'].cuda()
		last_row = torch.tensor([0, 0, 0, 1], dtype=cam.dtype, device=cam.device)
		last_row = last_row.view(1, 1, 1, 4).expand(cam.shape[0], cam.shape[1], -1, -1)
		cam = torch.cat([cam, last_row], dim=2)
		self.extras['cam'] = cam.view(-1,cam.shape[2],cam.shape[3])
		se3rep = torch.linalg.inv(cam) # [B, N, 4, 4]
		self.extras['se3fn'] = lambda A, x: torch.einsum(
            'bnij,bhntcj->bhntci', A, x)
		self.extras['se3rep_q'] = self.extras['se3rep_k'] = se3rep
		self.extras['inv_se3rep_q'] = cam
		self.depth = inputs['depth'].view(-1,inputs['depth'].shape[2],inputs['depth'].shape[3],inputs['depth'].shape[4]).cuda()
		zipped_lists2 = zip(*inputs['cls_name'])
		self.cls_name = [item for sublist in zipped_lists2 for item in sublist]
  
		self.anomaly = torch.stack(inputs['anomaly'], dim=1).view(-1)
		zipped_lists = zip(*inputs['img_path'])
		self.img_path = [item for sublist in zipped_lists for item in sublist]
		self.bs = self.imgs.shape[0]
		self.sample_anomaly = inputs['sample_anomaly']
	
	def forward(self):
		self.feats, self.feats_pred = self.net(self.imgs, self.extras, self.depth)
