...
class BaseTrainer():
    def __init__(self, cfg):
        ...
        if self.cfg.fvcore_is:
            dummy_imgs = torch.randn(5, self.cfg.fvcore_c, self.cfg.size, self.cfg.size).cuda()
            dummy_extras = {
                'cam':torch.randn(5, 4, 4).cuda(),
                'se3rep_q': torch.randn(1, 5, 4, 4).cuda(),
                'se3rep_k': torch.randn(1, 5, 4, 4).cuda(),
                'inv_se3rep_q': torch.randn(1, 5, 4, 4).cuda()
                }
            dummy_depth = torch.randn(5, 1, 518, 518).cuda()
            model_inputs = (dummy_imgs, dummy_extras, dummy_depth)
            print_networks([self.net], model_inputs, self.logger)
